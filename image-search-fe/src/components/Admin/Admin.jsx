...
const Admin = () => {
  ...
  const onAccept = id => () => dispatch(actions.replyReport({
    conlusion: "accept",
    id,
  }));

  const onReject = id => () => dispatch(actions.replyReport({
    conclusion: "reject",
    id,
  }));
              ...
              {rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                return (
                  <TableRow hover role="checkbox" tabIndex={-1} key={row.id}>
                    ...
                    {/* change to id */}
                    <TableCell className={classes.breedPhotoContainer} key={`${row.id}-photo`}>
                      <img className={classes.breedPhoto} src={row.photo} alt="Breed"></img>
                    </TableCell>
